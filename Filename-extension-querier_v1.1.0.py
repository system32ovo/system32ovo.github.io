...
/version或/ver 显示版本相关信息''')
        if c == '/list':
            print('当前版本收录的扩展名有：.txt, .docx, .doc, .pdf, .pptx, .ppt, .xlsx, .xls, .jpg, .jpeg, .png, .mp3, .mp4, .c, .cpp, .h, .java, .py, .html, .css, .js, .json, .xml, .php, .rb, .go, .swelift, .gelif, .pyw, .pyc, .jar, .zip, .rar, .7z, .tar, .exe, .dll, .app, .ipa, .apk, .deb, .rpm, .avi, .mov, .wav, .wma, .m4a, .ogg, .do, .webp, .svg, .url, .ani, .cur, .ini, .inf, .reg, .bat, .cmd, .sh, .iso, .img, .vhd, .vhdx, .hdd, .vmdk, .dmg, .pkg, .vmx, .vmsd, .plist, .log, .lnk, .vbs, .icns, .ico, .yml, .lock, .dat, .mca, .nbt, .properties, .conf, .dylib, .gz, .deny, .lzma, .qcow2, .vdi, .midi, .mid, .com, .rm, .ra, .rv, .ram, .sparseimage, .sb, .sb2, .sb3, .ai, .mxf, .a, .acm, .abf, .abk, .ace, .adf, .acp, .aps, .cif, .m4v, .cxx, .dcx, .dlg, .pak, .wim, .gho, .htm, .nws')
